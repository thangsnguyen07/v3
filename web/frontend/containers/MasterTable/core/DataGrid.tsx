...
export interface DataGridProps<R, SR = unknown, K extends Key = Key> extends SharedDivProps {
  ...
  columns: readonly Column<R, SR>[] | any
  /** A function called for each rendered row that should return a plain key/value pair object */
  rows: R[]
  /** Origin rows to compare */
  initialRows: R[]
   ...
   * Bottom horizontal scroll baronRowsChange can move the row left / right. Or a customized row renderer can be used to disabled the scrolling support.
   */
  summaryRows?: Maybe<readonly SR[]>
  /** The getter should return a unique key for each row */
  rowKeyGetter?: Maybe<(row: R | any) => K>
  onRowsChange?: Maybe<(rows: any, data: RowsChangeData<R, SR>) => void>
  ...
  rowGrouper?: Maybe<(rows: readonly R[], columnKey: string) => Record<string, any>>
...
function DataGrid<R, SR, K extends Key>(
   ...
   * defaults
   */
  if (!(rowHeight !== null && rowHeight !== void 0)) {
    rowHeight = 70
  }
  const headerRowHeight = rawHeaderRowHeight
    ? rawHeaderRowHeight - 20
    : typeof rowHeight === 'number'
    ? rowHeight
    : 50
  const summaryRowHeight = rawSummaryRowHeight ?? (typeof rowHeight === 'number' ? rowHeight : 50)
  const RowRenderer = rowRenderer ?? RowComponent
  const cellNavigationMode = rawCellNavigationMode ?? 'NONE'
  if (!(enableVirtualization !== null && enableVirtualization !== void 0)) {
  ...
  function getViewportRows() {
    ...
    for (let viewportRowIdx = startRowIdx; viewportRowIdx <= endRowIdx; viewportRowIdx++) {
      ...
      const row = rows[rowIdx]
      const checkedInitialRows = initialRows ? initialRows : []
      const initialRowIndex = checkedInitialRows.findIndex(
        r => r['id' as keyof R] === (row as R)['id' as keyof R]
      )
      const initialRow: R = checkedInitialRows[initialRowIndex]
...
export default forwardRef(DataGrid) as <R = SimpleProductRow, SR = unknown, K extends Key = Key>(
