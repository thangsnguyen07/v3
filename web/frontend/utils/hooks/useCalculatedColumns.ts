...
import type { DataGridProps } from 'containers/MasterTable/core'
import { SELECT_COLUMN_KEY } from 'containers/MasterTable/core/Columns'
import { ToggleGroupFormatter, ValueFormatter } from 'containers/MasterTable/core/formatters'

import { floor, max, min } from '../index'
...
export function useCalculatedColumns<R, SR>({
  ...
  const { columns, colSpanColumns, lastFrozenColumnIndex, groupBy } = useMemo((): {
    ...
    // FilterBar rawGroupBy and ignore keys that do not match the columns prop
    ...
    const columns = rawColumns.map(rawColumn => {
      ...
      if (rowGroup) {
        ...
        if (!(column.groupFormatter !== null && column.groupFormatter !== void 0)) {
          column.groupFormatter = ToggleGroupFormatter
        }
      }
